...
/*
 *  Last Changed Rev:  $Rev: 491 $
 *  Last Changed Date: $Date: 2013-04-22 13:44:02 +0900 (Mon, 22 Apr 2013) $
...
Returns:
	float
");		
    double getTriOhmicI(unsigned int tidx) const;
...
Returns:
    float
");		
    double getTriOhmicI(unsigned int tidx, std::string const & oc) const;
...
Returns:
    float
");		
    double getTriGHKI(unsigned int tidx) const;
...
Returns:
    float
");		
    double getTriGHKI(unsigned int tidx, std::string const & ghk) const;
...
"
	...
	%feature("autodoc", 
"
Sets the  electrical resistivity (in ohm/m^2) of the membrane with string identifier memb.
...
public:
    %feature("autodoc", "1");
    Tetexact(steps::model::Model * m, steps::wm::Geom * g, steps::rng::RNG * r, bool calcMembPot = false);
    %feature("autodoc", "1");
    ~Tetexact(void);
    %feature("autodoc", 
"
