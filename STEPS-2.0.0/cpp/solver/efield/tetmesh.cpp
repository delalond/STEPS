...
////////////////////////////////////////////////////////////////////////////////
/*
void sefield::TetMesh::displayMatrix(uint display)
{
	uint nverts = countVertices();
	...
	for (uint c = 0; c < ncons; ++c)
	{
		uint v0 = pConnections[c]->getA()->getIDX();
		uint v1 = pConnections[c]->getB()->getIDX();
		cmatrix[v0][v1] = 1.0;
		cmatrix[v1][v0] = 1.0;
	}

	if (display > nverts) display = nverts;

	// Messy if you try to display more than 100
	if (display > 100) display = 100;

	std::cout << "   ";
	for (uint i=0; i < display; ++i)
	{
		if (i < 10) std::cout << i << "  ";
		else if (i < 100) std::cout << i << " ";
	}
	std::cout << std::endl;
	for (uint row = 0; row < display; ++row)
	{
		std::cout << std::endl;
		if (row < 10) std::cout << row << "  ";
		else if (row < 100) std::cout << row << " ";

		for (uint col = 0; col < display; ++col)
		{
			...
			{
				if (col < 10) std::cout << "x  ";
				else if (col < 100) std::cout << "x ";
			}
		}
		std::cout << std::endl;
	}
