...
, pDiffBoundaries()
{
	...
	// Each 'bar' must connect at least 2 triangles, each tri has 3 bars TODO: better maximum
	uint barsn_max = trisn_max *2;
	uint * bars_temp = new uint[barsn_max];
	...
	// Create a set of vectors to store the bars.
	std::set<std::vector<uint> > bars = std::set<std::vector<uint> >();

	//pTri_Bars = new uint[pTrisN*2];

	uint bars_added = 0;
	/// loop over all triangles and set pTri_areas and pTri_norms
	for (uint tri = 0; tri < pTrisN; ++tri)
	{
		assert(bars_added <= barsn_max);
		// create array for bar formed by edges (0,1)
		// will also sort for ease of comparison
		uint bar_vert0[2]= {pTris[tri*3], pTris[(tri*3)+1]};
		std::sort(bar_vert0, bar_vert0 + 2);

		// create array for bar formed by edges (0,2)
		uint bar_vert1[2] = {pTris[tri*3], pTris[(tri*3)+2]};
		std::sort(bar_vert1, bar_vert1 + 2);

		// create array for bar formed by edges (1,2)
		uint bar_vert2[2] = {pTris[(tri*3)+1], pTris[(tri*3)+2]};
		std::sort(bar_vert2, bar_vert2 + 2);

		int bar0idx = -1;
		int bar1idx = -1;
		int bar2idx = -1;

		for (uint bar = 0; bar < bars_added; ++bar)
		{
			uint thisbar[2] = {bars_temp[bar*2], bars_temp[(bar*2)+1]};
			if (bar0idx == -1 && array_srt_cmp(bar_vert0, thisbar, 2)) bar0idx = bar;
			if (bar1idx == -1 && array_srt_cmp(bar_vert1, thisbar, 2)) bar1idx = bar;
			if (bar2idx == -1 && array_srt_cmp(bar_vert2, thisbar, 2)) bar2idx = bar;
		}

		// Add the bars that are not already included
		if (bar0idx == -1)
		{
			bars_temp[bars_added*2] = bar_vert0[0];
			bars_temp[(bars_added*2)+1] = bar_vert0[1];
			bar0idx = bars_added++;
		}
		if (bar1idx == -1)
		{
			bars_temp[bars_added*2] = bar_vert1[0];
			bars_temp[(bars_added*2)+1] = bar_vert1[1];
			bar1idx = bars_added++;
		}
		if (bar2idx == -1)
		{
			bars_temp[bars_added*2] = bar_vert2[0];
			bars_temp[(bars_added*2)+1] = bar_vert2[1];
			bar2idx = bars_added++;
		}

		pTri_bars[tri*3] = bar0idx;
		pTri_bars[(tri*3)+1] = bar1idx;
		pTri_bars[(tri*3)+2] = bar2idx;
	...
	// We know know how many bars are in the mesh
	pBarsN = bars_added;
	pBars = new uint[pBarsN * 2];
	// copy the supplied triangles information to pBars member
	for (uint i = 0; i < pBarsN*2; ++i) pBars[i] = bars_temp[i];
...
stetmesh::Tetmesh::~Tetmesh(void)
{
	...
	while (pMembs.empty() == false)
    {
        MembPMapCI memb = pMembs.begin();
        delete(memb->second);
    }
	while (pDiffBoundaries.empty() == false)
    {
        DiffBoundaryPMapCI diffb = pDiffBoundaries.begin();
        delete(diffb->second);
    }
}
...
std::vector<uint> stetmesh::Tetmesh::getBar(uint bidx) const
{
	...
	{
		std::ostringstream os;
		os << "Bar index is out of range.\n";
		throw steps::ArgErr(os.str());
	}
	std::vector<uint> bar(2);
	bar[0] = pBars[bidx*2];
	bar[1] = pBars[(bidx*2)+1];
	return bar;
}
...
std::vector<uint> stetmesh::Tetmesh::getTriBars(uint tidx) const
{
	...
	std::vector<uint> bars(3);
	bars[0] = pTri_bars[tidx*3];
	bars[1] = pTri_bars[(tidx*3)+1];
	bars[2] = pTri_bars[(tidx*3)+2];
	return bars;
}
...
std::vector<int> stetmesh::Tetmesh::getTriTriNeighb(uint tidx, stetmesh::TmPatch * tmpatch) const
{
	assert(pSetupDone == true);
	if (tidx >= pTrisN)
	{
	...
	// Create the set of size 3 that will store the triangles neighbours
	std::vector<int> neighbours = std::vector<int>(3);
	neighbours[0]=-1;
	neighbours[1]=-1;
	neighbours[2]=-1;

	std::vector<uint> bars = getTriBars(tidx);

	for (uint tri = 0; tri < pTrisN; ++tri)
	{
		if (tri == tidx) continue;
		if (getTriPatch(tri) == tmpatch)
		{
			std::vector<uint>  neighbtribars = getTriBars(tri);
			bool found = false;
			for (uint i = 0; i < 3; ++i)
			{
				if (found == true) break;
	    		for (uint j=0; j<3; ++j)
				{
	    			if (neighbtribars[j] == bars[i])
	    			{
	    				if (neighbours[i] != -1)
	    				{
	    	    	        ...
	    	    	        os <<"'. Patch triangle idx " << tidx << " found to have more than 3 neighbours. \n";
	    	    	        throw steps::ArgErr(os.str());
	    	    	    }
	    				neighbours[i] = tri;
	    				found = true;
...
std::set<uint> stetmesh::Tetmesh::getTriTriNeighbs(uint tidx) const
{
	...
	// Create the set of unknown size that will store the triangles neighbours
	std::set<uint> neighbours = std::set<uint>();
	...
	{
		if (tri == tidx) continue;
		std::vector<uint> tribarstemp = getTriBars(tri);
		if (stetmesh::bar_vec_contains(bars, tribarstemp)) neighbours.insert(tri);
	}
...
bool stetmesh::bar_vec_contains(std::vector<uint> bars0, std::vector<uint> bars1)
{
	uint bar00 = bars0[0];
	if (bar00 == bars1[0] or bar00 == bars1[1] or bar00 == bars1[2]) return true;
	uint bar01 = bars0[1];
	if (bar01 == bars1[0] or bar01 == bars1[1] or bar01 == bars1[2]) return true;
	uint bar02 = bars0[2];
	if (bar02 == bars1[0] or bar02 == bars1[1] or bar02 == bars1[2]) return true;
